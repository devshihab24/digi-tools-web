...
const PricingCard = ({ plan }) => {
  ...
  return (
    <div className={`relative p-5 space-y-5 rounded-xl card-shadow flex flex-col ${popular ? 'text-white bg-linear-to-r from-[#4F39F7] to-[#9b4d99]' : ''}`}>
        {
      ...
      <button className={`w-full mt-auto ${popular ? 'pricing-btn' : 'common-btn'} `}>{purchaseTypeBtn}</button>
    </div>
  );
};
